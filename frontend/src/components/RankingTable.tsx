...
export function RankingTable({ rows }: { rows: RankingRow[] }) {
  ...
  return (
    <div className="min-w-0 divide-y divide-white/[0.07]">
      {rows.map((item, index) => (
        <article key={item.candidateId} className="grid min-h-[76px] min-w-0 grid-cols-[52px_180px_minmax(0,1fr)_110px] items-center gap-5 max-md:grid-cols-[44px_minmax(0,1fr)_96px]">
          ...
          <strong className="min-w-0 truncate text-right font-mono text-2xl font-black">{formatCount(item.votes)}</strong>
        </article>
      ))}
    </div>
  );
