  ...
  return (
    <div className="grid">
      {items.map((item, index) => {
        ...
        return (
          <div key={`${item.title}-${index}`} className="relative grid grid-cols-[42px_minmax(0,1fr)_auto] gap-3 border-b border-white/[0.07] py-4 last:border-b-0">
            ...
            <div className="text-xs text-ops-muted">{item.right}</div>
          </div>
        );
      })}
    </div>
