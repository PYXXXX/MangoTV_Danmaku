...
flowchart LR
    ...
    OP["运营浏览器"] -->|HTTPS + 登录密码| NGINX["Nginx / 可信代理"]
    NGINX --> SVC
    FS["飞书运营群"] <-->|出站 WebSocket 长连接| SVC
    SVC --> DATA["server/data<br/>状态、原始弹幕、SQLite 去重"]
...
- 原始弹幕、昵称、数据库、`server/config.json` 不上传 GitHub。
...
~~~bash
sudo -u mgtv-vote cp server/config.example.json server/config.json
sudo -u mgtv-vote chmod 600 server/config.json
sudo -u mgtv-vote mkdir -p server/data
~~~

`server/config.json`、`server/data/` 和配置备份均已被 Git 忽略。
...
~~~bash
sudoedit /opt/MangoTV_Danmaku/server/config.json
~~~
...
~~~json
{
  "mgtv": {
    ...
    "dedup_db_path": "server/data/fingerprints.sqlite3"
  }
}
~~~
...
- `server/data/state.json`：活动、场次、票数和发布状态。
- `server/data/raw_messages.jsonl`：全量去重后的原始弹幕日志。
- `server/data/rounds/*.jsonl`：按场次追加的记录。
- `server/data/fingerprints.sqlite3`：持久化去重索引。
...
~~~bash
sudo -u mgtv-vote .venv/bin/python tools/setup_operator_password.py --session-hours 12 --secure-cookie
~~~
...
~~~bash
sudo -u mgtv-vote .venv/bin/python tools/setup_operator_password.py --disable
~~~
...
正式节目不要保留 `*`；两个白名单都为空也会兼容性地允许所有可触达 Bot 的用户操作。
...
~~~bash
cd /opt/MangoTV_Danmaku
sudo -u mgtv-vote .venv/bin/python server/vote_server.py --config server/config.json
~~~
...
服务示例启用了 `ProtectSystem=full`，只允许服务写入 `server/data`。配置向导应在服务外执行；执行后重启服务读取新配置。

## 9. HTTPS 反向代理

正式环境建议让服务只监听 `127.0.0.1:8080`，由 Nginx 终止 TLS。
...
- [ ] Python 依赖安装成功。
- [ ] `server/config.json` 权限为 600，所有者为服务用户。
- [ ] `server/data` 可写，其他源码目录不需要写权限。
- [ ] `/healthz` 返回 200。
- [ ] 创建、结束、重命名、导出场次均正常。
- [ ] 停止并重启服务后，历史场次仍存在。
- [ ] 初始历史弹幕没有被误算入新场次。
...
sudo tar -czf /var/backups/mgtv-danmaku-<DATE>.tar.gz server/config.json server/data
...
sudo systemctl stop mgtv-danmaku
cd /opt/MangoTV_Danmaku
sudo tar -xzf /var/backups/mgtv-danmaku-<DATE>.tar.gz
sudo chown -R mgtv-vote:mgtv-vote server/config.json server/data
sudo chmod 600 server/config.json
sudo systemctl start mgtv-danmaku
sudo systemctl status mgtv-danmaku
~~~
...
保留 `server/config.json` 和 `server/data`，只切换代码：
...
~~~bash
.venv/bin/python tools/export_round_slice.py   --data-dir server/data   --round "第一轮"   --out output/round-1.jsonl
~~~
...
- `server/config.json` 是否为合法 JSON。
- Python 依赖是否安装在项目 `.venv`。
- 8080 是否已被占用。
- `server/data` 与 SQLite 文件是否归服务用户所有。
...
~~~bash
sudo -u mgtv-vote .venv/bin/python tools/setup_operator_password.py --insecure-cookie
sudo systemctl restart mgtv-danmaku
~~~
