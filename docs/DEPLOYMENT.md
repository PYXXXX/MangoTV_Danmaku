...
- 服务器时区不影响数据格式；场次名称保持运营命名，采集时间范围作为独立字段按北京时间展示。
...
随后把 Bot 添加到运营群，在群里发送“我的ID”获取 `chat_id`，填入 WebUI 的 `allowed_chat_ids` 后保存。正式节目不要保留 `*`；生产版本不再允许两个白名单都为空时执行运营操作，白名单外用户只能发送“我的ID”获取自身 ID。

这个流程复用了飞书官方 `larksuite/cli config init --new` 的授权注册方式。如果企业租户策略阻止自动注册，WebUI 会显示失败原因，此时使用下面的手动兜底方式。

注意：一键绑定生成的应用可能是飞书 CLI/PersonalAgent 风格应用，适合快速拿到凭据并发送消息，但不一定会出现在企业开放平台控制台里。如果卡片按钮提示“尚未配置卡片回调”，点击“去配置”又提示“该应用不存在”，说明当前凭据对应的应用不可在租户控制台管理。完整卡片按钮交互必须改用下面的企业自建应用方式，并配置 `card.action.trigger`。
...
6. 添加卡片回调 `card.action.trigger`。缺少这个回调时，卡片能显示，但任何按钮或下拉选择都会在飞书客户端提示“尚未配置卡片回调”。
7. 创建并发布应用版本；按企业流程完成管理员审核。
8. 把 Bot 加入运营群，或直接与 Bot 私聊。

如果在飞书客户端点击“去配置”提示“该应用不存在”，通常表示当前 `app_id/app_secret` 来自 CLI 一键注册的 PersonalAgent，不是企业开放平台里的自建应用；请改用企业自建应用的 `app_id/app_secret`，在 WebUI 手动填入并保存热重载。
...
发送任意消息都会刷新运营控制台卡片；推荐直接使用卡片按钮，不需要记忆文本命令。卡片支持：
...
| GET | `/healthz` | 是 | 最小健康检查，未登录只返回 `{ "ok": true }` |
