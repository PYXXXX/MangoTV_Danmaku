...
- `GET /api/update/status`：检查当前 commit 与远端 commit。
- `POST /api/update/apply`：空闲时快进升级、更新依赖并自动重启。
...
登录后点击“系统配置”可在线维护活动、候选人、别名、多人策略、直播源、采集/去重、GitHub、飞书与密码策略，并可检查远端新 commit 后一键升级程序。大多数配置立即热应用；当前场次的候选人口径保持冻结，监听地址、端口、数据目录、SQLite 路径和程序升级由界面提示安全重启后生效。
