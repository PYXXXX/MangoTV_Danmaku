...
一个集直播录制、弹幕采集、数据分析、结果发布和飞书协同于一体的直播运营工具。
...
2. **直播录制**：支持芒果 TV 扫码登录、清晰度选择、播放源检测，并用 ffmpeg 录制完整直播视频。
3. **弹幕留档**：同时保存处理后弹幕 JSONL 和原始观测弹幕 JSONL，便于审计、回放和后处理。
4. **录制后处理**：运营无法实时盯直播时，可回看视频、打标、截取片段，再导出片段弹幕或生成分析场次。
5. **数据分析**：本地规则先处理明确票，歧义弹幕可交给 Codex 按规范清洗，最后上传精确结果。
6. **结果发布**：聚合票数可同步到 GitHub Pages 公开页，支持导出 PNG；不会公开昵称和原始弹幕。
7. **协同管理**：飞书 Bot 提供卡片式远程操作，运营 WebUI 支持密码保护、热配置和一键升级。
...
运营服务器适合正式直播：不打开浏览器、不依赖前端电脑性能，直接轮询客户端同款弹幕接口并本地去重计票；启用录制后还会同步保存完整视频和弹幕轨。
...
启动后打开 `http://服务器地址:8080/` 即可进入新版运营 Studio，风格与公开结果页一致，可按“活动监控”“运营工作区”“系统配置”“机器状态”“系统日志”五个页面工作：开始/结束场次、重命名、删除、发布粗略结果、查看排名、导出切片、回看录屏、截取片段，以及上传并发布清洗后的精确结果。旧版 WebUI 仅保留在 `/legacy` 作为临时备用入口。
...
推荐路径：打开运营 Studio → “系统配置” → “连接与账号” → “飞书一键绑定”，点击授权链接并按飞书页面完成授权/安装。绑定成功后系统会自动保存 `app_id` / `app_secret`、启用 WebSocket 长连接并热重载；`app_secret` 不会回显到页面。授权人的 `open_id` 会自动加入白名单，便于立即私聊 Bot 测试。
...
随后把 Bot 加入运营群，在群里发送“我的ID”获取 `chat_id`，填入 Studio 的 `allowed_chat_ids` 后保存。向 Bot 发送任意消息即可刷新运营控制台卡片，可点击开始默认场次、结束、刷新、查看/切换场次、导出 PNG、删除和发布粗略结果。自定义活动名、候选人与直播源建议在 Studio 配置；精确结果文件仍在运营 Studio 上传。
...
2. 点击“结束场次”，系统锁定 JSONL 切片，并把北京时间范围作为独立时间信息保存。
